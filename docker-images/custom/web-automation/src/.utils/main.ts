...
export async function runAutomation(callback: (page: Page) => Promise<void>, _options: { date: string }) {
    ...
    try {
        ...
        try {
            ...
            await page.screenshot({ fullPage: false, path: path.join(options.errorDir, `${options.date}-viewport.png`) });
            await page.screenshot({ fullPage: true, path: path.join(options.errorDir, `${options.date}-fullpage.png`) });
            throw error;
        } finally {
            await page.close();
        }
