...
DOCKER_COMPONENTS := $(shell printf 'components/database components/database-backup components/http-proxy components/lamps/hardware-controller components/lamps/network-server components/omada-controller components/pihole components/smtp4dev components/socat components/unifi-controller components/uptime-kuma components/webui-backup' | sed 's~ ~\n~g')
