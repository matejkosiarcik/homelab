...
DOCKER_COMPONENTS := $(shell printf 'components/socat components/http-proxy components/webui-backup components/lamps/hardware-controller components/lamps/network-server components/pihole machines/odroid-h3/healthchecks/database-backup' | sed 's~ ~\n~g')
