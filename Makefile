...
.PHONY: bootstrap
bootstrap:
	printf '%s' "$(NPM_COMPONENTS_ALL)" | base64 -d --ignore-garbage | while read -r component; do \
...
.PHONY: build
build:
	printf '%s' "$(NPM_COMPONENTS_FOR_BUILD)" | base64 -d --ignore-garbage | while read -r component; do \
		printf 'Building %s\n' "$$component" && \
...
.PHONY: docker-build
docker-build:
	printf '%s' "$(DOCKER_IMAGES)" | base64 -d --ignore-garbage | while read -r component; do \
		printf 'Building %s\n' "$$component" && \
		docker build "$(PROJECT_DIR)/docker-images" --file "$(PROJECT_DIR)/$$component/Dockerfile" --tag "$$(printf '%s\n' "$$component" | tr '/' '-' | tr -d '.'):homelab" && \
		printf '\n\n' && \
	true; done

	printf '%s' "$(DOCKER_APPS)" | base64 -d --ignore-garbage | while read -r app; do \
		printf 'Building %s\n' "$$app" && \
...
docker-build-multiarch:
	printf '%s' "$(DOCKER_ARCHS)" | base64 -d --ignore-garbage | while read -r arch; do \
		printf '%s' "$(DOCKER_IMAGES)" | base64 -d --ignore-garbage | while read -r component; do \
