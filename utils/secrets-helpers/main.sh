...
adventurelog)
    ...
    load_secret ".$app_full_name_key.postgis.ca_certificate" "dev=value=$(cat "$tmpdir/postgres-dev.crt" | base64)" | base64 -d >"$initial_output/postgres.crt"
    load_secret ".$app_full_name_key.postgis.ca_private_key" "dev=value=$(cat "$tmpdir/postgres-dev.key" | base64)" | base64 -d >"$initial_output/postgres.key"
...
git-cache)
    ...
    openssl req -new -x509 -days 3650 -nodes -text -out "$tmpdir/postgres-dev.crt" -keyout "$tmpdir/postgres-dev.key" -subj '/CN=postgres'
    load_secret ".$app_full_name_key.postgres.ca_certificate" "dev=value=$(cat "$tmpdir/postgres-dev.crt" | base64)" | base64 -d >"$initial_output/postgres.crt"
    load_secret ".$app_full_name_key.postgres.ca_private_key" "dev=value=$(cat "$tmpdir/postgres-dev.key" | base64)" | base64 -d >"$initial_output/postgres.key"
    rm -f "$tmpdir/postgres-dev.crt" "$tmpdir/postgres-dev.key"
