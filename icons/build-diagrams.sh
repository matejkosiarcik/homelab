...
cat "$input_dir/gitman-repositories/dia-vrt-sheets/SVG/VRT Clients & Peripherals/Laptop 3.svg" | perl -0pe 's~<g id="id3">.+?</g>~~gms' >"$tmpdir/notebook.svg"
convert_image_full "$tmpdir/notebook.svg" "$output_dir/notebook.png"
rm -f "$tmpdir/notebook.svg"

cat "$input_dir/gitman-repositories/dia-vrt-sheets/SVG/VRT Networking & Communications/Router.svg" | perl -0pe 's~<g id="id3">.+?</g>~~gms' >"$tmpdir/router.png"
convert_image_full "$tmpdir/router.png" "$output_dir/router.png"
rm -f "$tmpdir/router.png"

cat "$input_dir/gitman-repositories/dia-vrt-sheets/SVG/VRT Networking & Communications/Switch 2.svg" | perl -0pe 's~<g id="id3">.+?</g>~~gms' >"$tmpdir/switch.png"
convert_image_full "$tmpdir/switch.png" "$output_dir/switch.png"
rm -f "$tmpdir/switch.png"

cat "$input_dir/gitman-repositories/dia-vrt-sheets/SVG/VRT Networking & Communications/Wireless Access Point 2.svg" | perl -0pe 's~<g id="id3">.+?</g>~~gms' >"$tmpdir/wifi-ap.png"
convert_image_full "$tmpdir/wifi-ap.png" "$output_dir/wifi-ap.png"
rm -f "$tmpdir/wifi-ap.png"

cat "$input_dir/gitman-repositories/dia-vrt-sheets/SVG/VRT Servers/Tower Server 1.svg" | perl -0pe 's~<g id="id3">.+?</g>~~gms' >"$tmpdir/server-big.png"
convert_image_full "$tmpdir/server-big.png" "$output_dir/server-big.png"
rm -f "$tmpdir/server-big.png"

cat "$input_dir/gitman-repositories/dia-vrt-sheets/SVG/VRT Servers/Appliance 1.svg" | perl -0pe 's~<g id="id3">.+?</g>~~gms' >"$tmpdir/server-small.png"
